      ...
      else
      {
        ...
        if (robot_msg.has_perception())
        {
          ...
          if (perception.has_ball_in_self())
          {
            ...
            std::cout << "\tBall position: (" << ball.x() << ", " << ball.y() << ") (quality: " << perception.ball_quality() << ")" << std::endl;
          }
          else
          {
          ...
          if (perception.has_self_in_field())
          {
            ...
            std::cout << "\tSelf position: (" << self.position().x() << ", " << 
                self.position().y() << "), dir: " << self.dir() <<" quality: "<<perception.field_quality()<< std::endl;
          }
          else
          {
          ...
          if (perception.robots_size() > 0)
          {
            ...
            for (int i = 0; i < perception.robots_size(); i++)
            {
              ...
              std::cout << "\t - Robot " << i << ": (" << robot.position().x() << ", " << robot.position().y() << ")" << std::endl;
            }
          }
          else
          {
        ...
        if (robot_msg.has_captain())
        {
          ...
          if (captain.has_ball())
          {
            ...
            std::cout << "\tCaptain ball position: (" << ball.x() << ", " << ball.y() << ") (votes: " << captain.ball().nb_votes() << ")" << std::endl;
          }
          else
          {
          ...
          for(int i=0; i<captain.opponents_size(); i++){
            ...
            std::cout << "\t - Opponent " << i << ": (" << opponent.x() << ", " << opponent.y() << ") (votes: " << captain.opponents(i).nb_votes() << ")" << std::endl;
          }
          ...
          for (int i = 0; i < captain.orders_size(); i++)
          {
            ...
            std::cout << "\t - Order for robot " << order.robot_id() << ": action=" << hl_communication::Action_Name(order.action()) << std::endl;
            if (order.has_target_pose())
            {
              std::cout << "\t\ttarget=(x=" << order.target_pose().position().x() << " y=" << order.target_pose().position().y()
                        << " dir=" << order.target_pose().dir() << ")";
            }
            std::cout << std::endl;
          }
        }
